            ...
            if i >= 0:
                ...
                with torch.no_grad():
                    ...
                    if len(bboxes_prev) > 0:
                        ...
                        node_embeds_prev = torch.tensor([]).cuda()
                ...
                for g in range(len(len_graphs)):
                    ...
                    node_embeds_g = torch.stack([node_embeds[j] for j in data_df[g]['node']])
                    ...
                    edge_ixs_g_np = utils.apply_density_aware_sparsification(
                    ...
                    if prev_num_nodes > 0 and len(data_df[g]) > 0:
                        ...
                        points_prev = np.stack((data_df_prev[g]['xw'].values, data_df_prev[g]['yw'].values), axis=-1)
                    ...
                    data_obj = Data(x=node_embeds_g, edge_index=edge_ixs_g, y=node_label_g, edge_attr=edge_attr,
                                    edge_labels=edge_labels_g)


                    data_obj.prev_x = node_embeds_prev_g
                    data_obj.temporal_edge_index = temporal_edge_index

                    if temporal_edge_index.numel() > 0:
                        data_obj.prev_edge_attr = torch.zeros((temporal_edge_index.size(1), edge_attr.size(1))).cuda()
                    else:
                        data_obj.prev_edge_attr = torch.zeros((0, edge_attr.size(1))).cuda()
                ...
                raw_logits = outputs['classified_edges'][-1].view(-1)
                preds_prob = torch.sigmoid(raw_logits)
                ...
                predictions_tensor = (preds_prob >= threshold).float()
                ...
                GT_active_edges = [(edge_list[0][pos], edge_list[1][pos]) for pos, p in enumerate(labels_edges_GT) if
                                   p == 1]
                G_GT = nx.DiGraph(GT_active_edges)
                ID_GT, n_clusters_GT = utils.compute_SCC_and_Clusters(G_GT, data_batch.num_nodes)

                predicted_active_edges = [(edge_list[0][pos], edge_list[1][pos]) for pos, p in enumerate(predictions) if
                                          p == 1]
                ...
                if CONFIG['ROUNDING']:
                    predictions_r = utils.compute_rounding(data_batch, predictions_tensor, preds_prob, predicted_active_edges)
                    if predictions_r != []:
                ...
                current_batch_df = pd.concat(data_df_t, ignore_index=True)
                current_batch_df['predicted_ID'] = ID_pred



                #dataset_name = 'EPFL-Basketball'
                #dataset_name = 'EPFL-Laboratory'
                dataset_name = 'EPFL-Terrace'

                save_path = f'./results/{dataset_name}_predictions.csv'

                if not os.path.exists(save_path):
                    current_batch_df.to_csv(save_path, index=False, mode='w')
                else:
                    current_batch_df.to_csv(save_path, index=False, mode='a', header=False)
