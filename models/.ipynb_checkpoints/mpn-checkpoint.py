...
class MOTMPNet(nn.Module):
    ...
    def __init__(self, model_params, bb_encoder = None, arch=None):
        ...
        # [创新点植入]: 双分支特征融合模块 (Cross-Attention Fusion)
        # 负责动态评估“当前表观/几何”与“历史运动先验”的重要性并进行融合
        node_out_dim = encoder_feats_dict['node_out_dim']

        # 时序特征投影层 (将历史特征投影到当前空间)
        ...
        # 跨注意力融合门控 (Cross-Attention Gate)
        # 用来决定吸收多少时序历史信息，防止误差累积
    ...
    def _build_core_MPNet(self, model_params, encoder_feats_dict):
        ...
        # --- 分支 1: 空间分支 (Spatial Branch - 负责"是谁") ---
        ...
        # --- 分支 2: 时序分支 (Temporal Branch - 负责"去哪") ---
        # 时序边通常只传递节点运动状态，不需要复杂的边特征更新，故 edge_model 可设为 None 或简化
    ...
    def forward(self, data):
        # 注意: 我们假设 data 中现在多传了 prev_x (上一帧特征) 和 temporal_edge_index (时序边)
        x, edge_index, edge_attr = data.x, data.edge_index, data.edge_attr

        # 兼容性处理：如果是第一帧，没有时序边，做个特判
        ...
        # =====================================================================
        # [创新点植入]: 增强节点初始化
        # 如果有上一帧的特征，通过时序投影增强当前的初始特征
        if has_temporal:
            _, prev_latent_node_feats = self.encoder(data.prev_edge_attr, data.prev_x)
            prev_proj = self.temporal_proj(prev_latent_node_feats)
            # 通过 Gate 机制决定吸收多少先验
            gate = self.fusion_gate(torch.cat([latent_node_feats, prev_proj], dim=-1))
            latent_node_feats = gate * latent_node_feats + (1 - gate) * prev_proj
        # =====================================================================
        ...
        for step in range(1, self.num_enc_steps + 1):
            ...
            # 1. 空间分支前向传播 (处理跨相机 "是谁" 的问题)
            latent_node_feats_s, latent_edge_feats = self.spatial_MPNet(latent_node_feats, edge_index,
                                                                        latent_edge_feats)

            # 2. 时序分支前向传播 (处理跨帧 "去哪" 的运动平滑性问题)
            if has_temporal:
                # 沿时序边 (t-1 -> t) 传递消息
                # 注意：这里我们让时序分支的边特征传全0或共享空间边特征的维度，保持接口一致
                dummy_temporal_edge_attr = torch.zeros(data.temporal_edge_index.size(1), latent_edge_feats.size(1),
                                                       device=latent_edge_feats.device)
                latent_node_feats_t, _ = self.temporal_MPNet(prev_latent_node_feats, data.temporal_edge_index,
                                                             dummy_temporal_edge_attr)

                # 3. 双分支特征融合 (Parallel Dual-Branch Fusion)
                fusion_weight = torch.sigmoid(self.temporal_proj(latent_node_feats_t))
