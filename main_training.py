...
def load_model(CONFIG):
    ...
    if cnn_arch == 'resnet50':
        ...
        cnn_model = top_bdnet_neck_doubot(num_classes=751, loss='softmax', pretrained=True, use_gpu=True,
        ...
        cnn_model = top_bdnet_neck_botdropfeat_doubot(num_classes=767, loss='softmax', pretrained=True, use_gpu=True,
                                                      feature_extractor=True)
        load_pretrained_weights(cnn_model, CONFIG['CNN_MODEL']['model_weights_path'][cnn_arch])
        cnn_model.eval()
        ...
        multiheads_baseline.Baseline.load_pretrained_weights(cnn_model,
                                                             CONFIG['CNN_MODEL']['model_weights_path'][cnn_arch])
        cnn_model.eval()
...
def my_collate(batch):
    bboxes_t_batches = [item[0] for item in batch]
    df_t_batches = [item[1] for item in batch]
    bboxes_prev_batches = [item[2] for item in batch]
    df_prev_batches = [item[3] for item in batch]
    max_dist = [item[4] for item in batch]

    return [bboxes_t_batches, df_t_batches, bboxes_prev_batches, df_prev_batches, max_dist]
...
validation_loader = torch.utils.data.DataLoader(val_dataset, batch_size=CONFIG['TRAINING']['BATCH_SIZE']['VAL'],
                                                shuffle=False,
                                                num_workers=CONFIG['DATALOADER']['NUM_WORKERS'], collate_fn=my_collate,
                                                pin_memory=CONFIG['DATALOADER']['PIN_MEMORY'])
...
if CONFIG['TRAINING']['LOSS']['NAME'] == 'Focal':
    ...
    criterion = nn.BCEWithLogitsLoss(reduction='mean', pos_weight=torch.tensor(
        CONFIG['POSITIVE_WEIGHT'][CONFIG['DATASET_TRAIN']['NAME'][0]]))
    criterion_no_reduction = nn.BCEWithLogitsLoss(reduction='none', pos_weight=torch.tensor(
        CONFIG['POSITIVE_WEIGHT'][CONFIG['DATASET_TRAIN']['NAME'][0]]))
...
for epoch in range(0, CONFIG['TRAINING']['EPOCHS']):
    ...
    train_losses, train_losses1, train_losses0, train_precision_1, train_precision_0, train_loss_in_history, \
        ...
        train(CONFIG, train_loader, cnn_model, mpn_model, epoch, optimizer, results_path, train_loss_in_history, \
              train_prec1_in_history, train_prec0_in_history, train_prec_in_history, train_dataset, dataset_dir,
              criterion, criterion_no_reduction, list_mean_probs_history)
    ...
    val_losses, val_losses1, val_losses0, val_precision_1, val_precision_0, val_loss_in_history, val_prec1_in_history, val_prec0_in_history, val_prec_in_history, list_mean_probs_history_val = \
        validate(CONFIG, validation_loader, cnn_model, mpn_model, results_path, epoch, val_loss_in_history,
                 val_prec1_in_history, val_prec0_in_history, val_prec_in_history,
        ...
        if (flag_warmup_ended):
            ...
            if CONFIG['TRAINING']['LR_SCHEDULER']['type'] == 'STEP':
                scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
                                                            step_size=CONFIG['TRAINING']['LR_SCHEDULER']['args'][
                                                                'step_size'],
    ...
    plt.plot((np.asarray(training_precision_1_avg) + np.asarray(training_precision_0_avg)) / 2, '--',
             label='Training MCA')
    ...
    try:
        log_df = pd.DataFrame({
            ...
            'Val_Prec_Class1': val_precision_1_avg,
            'Val_Prec_Class0': val_precision_0_avg
            })
        csv_path = os.path.join(results_path, 'files', 'training_log.csv')
        log_df.to_csv(csv_path, index=False)

    except ValueError as e:
        print(f"CSV保存失败，可能是列表长度不匹配: {e}")
