...
def apply_homography_image_to_world(xi, yi, H_image_to_world):
    ...
    S = np.array([xi, yi, 1], dtype=object).reshape(3, 1)
...
def remove_edges_single_direction(active_edges, predictions, edge_list):
    ...
    if idx_active_edges_to_remove != []:
        ...
        global_idx_edges_to_remove = np.asarray(predicted_active_edges_global_pos)[
            np.asarray(idx_active_edges_to_remove)]
...
def apply_density_aware_sparsification(edge_ixs_np, data_df_g, prev_max_counter, eta=0.5, k_min=2, k_max=10):
