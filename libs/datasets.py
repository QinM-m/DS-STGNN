...
class EPFL_dataset(Dataset):
    ...
    def __init__(self, name, mode, CONFIG, cnn_model):
        ...
        if mode == 'train':
            ...
            self.data_det['frame'] = (self.data_det['frame'].values).astype(int)
    ...
    def __getitem__(self, index):
        ...
        frame_t = self.frames_valid[index]
        data_f_t = self.data_det[self.data_det['frame'] == frame_t]
        cams_t = np.unique(data_f_t['id_cam'].values)


        if index == 0:
            frame_prev = frame_t
        else:
            frame_prev = self.frames_valid[index - 1]
        data_f_prev = self.data_det[self.data_det['frame'] == frame_prev]
        cams_prev = np.unique(data_f_prev['id_cam'].values)


        def get_bboxes_for_frame(cams, data_f, frame):
            img_f = list()
            for i in range(len(self.cameras)):
                if i in cams:
                    c = self.cameras[i]
                    frame_path = os.path.join(self.path, c, 'img1', str(frame).zfill(6) + '.jpg')
                    img_f.append(imread(frame_path))
                else:
                    img_f.append([])
        ...
        bboxes_t = get_bboxes_for_frame(cams_t, data_f_t, frame_t)
        bboxes_prev = get_bboxes_for_frame(cams_prev, data_f_prev, frame_prev)


        return [bboxes_t, data_f_t, bboxes_prev, data_f_prev, self.max_dist]
